...
{
	printf("Win32Thread Created!\n");
	thread_param* tp = (thread_param*)pParam;
	if (tp == NULL)
	{
		return 1L;
	}

	(DWORD)tp->callback(tp->param);
	free(tp);
	tp = NULL;
...
uint32_t calc_crc(const char* local_path, uint32_t offset, uint32_t length)
{
	...
	FILE* pf = fopen(local_path, "rb");
	if (NULL != pf)
	{
		// 获取文件大小
		fseek(pf, 0, SEEK_END);
		uint32_t nFileSize = (uint32_t)ftell(pf);
		if ( offset + length > nFileSize)
		{
			return 0;
		}
		...
		unsigned int nBuffSize = (length < 10240000) ? length : 10240000;
		unsigned char* pBuffer = new unsigned char[nBuffSize];	// 10MB(大文件分块处理, 以防止因申请内存过大而失败)
		if (NULL == pBuffer)
		{
			return 0;
		}
		...
		while (nFileSize > 0)
		{
			...
			nReadBytes = fread(pBuffer, 1, nBuffSize, pf);
			// calc...
			for (unsigned int i = 0; i < nReadBytes; i++)
			{
				nCRC32 = (nCRC32 >> 8) ^ CRC32Table[(nCRC32 & 0xFF) ^ pBuffer[i]];
			}
...
uint32_t calc_crc(const char* local_path)
{
	...
	if (NULL != pf)
	{
		// 获取文件大小
		fseek(pf, 0, SEEK_END);
		unsigned int nFileSize = (unsigned int)ftell(pf);
		fseek(pf, 0, SEEK_SET);

		unsigned int nCRC32 = 0xFFFFFFFF;
		unsigned int nReadBytes = 0;
		unsigned int nBuffSize = (nFileSize < 10240000) ? nFileSize : 10240000;
		unsigned char* pBuffer = new unsigned char[nBuffSize];	// 10MB(大文件分块处理, 以防止因申请内存过大而失败)
		if (NULL == pBuffer)
		{
			return 0;
...
int SplitFiles(const char* file, uint32_t block_size, char** files)
{
	...
	if(stat(file, &file_info) != 0 || file_info.st_size == 0) 
	{
		return 0;
	}
	int nblocks = 1;
	if (file_info.st_size > block_size)
	{
		nblocks = file_info.st_size/block_size;
		if (file_info.st_size%block_size)
		{
			nblocks+=1;
		}
		FILE* f_origin = fopen(file, "rb+");
		if ( f_origin == NULL )
		{
			return 0;
		}
